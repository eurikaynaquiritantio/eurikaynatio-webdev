...
<head>
    ...
    <title>Best Sellers | TIO Perfume Collection</title>
...
<header class="site-header">
    ...
    <nav class="main-nav">
        ...
        <a href="best-sellers.php" class="active">BEST SELLERS</a>
...
<main>
    ...
    <section class="page-hero">
        ...
        <p class="breadcrumb"><a href="index.php">Home</a><span> / </span>Best Sellers</p>
    </section>

    <section class="section">
        <?php if (empty($products)): ?>
            <p style="text-align:center;font-size:12px;">No bestsellers marked yet — set is_bestseller/bestseller_rank on a product to feature it here.</p>
        <?php else: ?>
        <div class="product-grid">
            <?php foreach ($products as $product): ?>
            <article class="product-card ranked<?php echo $product['is_out_of_stock'] ? ' out-of-stock' : ''; ?>">
                <span class="rank-badge"><?php echo (int)$product['bestseller_rank']; ?></span>
                ...
                <?php else: ?>
                    <select class="variant-select" aria-label="Choose size for <?php echo htmlspecialchars($product['name']); ?>">
                        <?php foreach ($product['available_variants'] as $variant): ?>
                            <option value="<?php echo (int)$variant['id']; ?>"><?php echo htmlspecialchars($variant['size_label']); ?> · <?php echo (int)$variant['stock_qty']; ?> left</option>
                        <?php endforeach; ?>
                    </select>
